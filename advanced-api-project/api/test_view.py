...
class BookAPITestCase(APITestCase):
    def setUp(self):
        ...
        # Sample books
        self.book1 = Book.objects.create(title="The Alchemist", author="Paulo Coelho", publication_year=1993)
        self.book2 = Book.objects.create(title="1984", author="George Orwell", publication_year=1949)

        # Endpoints
        self.list_url = reverse("book-list")  # DRF ViewSet auto-route
    ...
    def test_filter_books_by_author(self):
        response = self.client.get(self.list_url + "?author=Paulo Coelho")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]["author"], "Paulo Coelho")
